...
This app is a Flutter app (`app/`) with two embedded language engines:
...
Course content lives in `courseware/` and is copied into `app/assets/courseware/`
by the build script. `tools/export_content.py` then flattens the two courses into
a single `app/assets/courseware/pack.json` that the Flutter app loads (the asset
...
```bash
cd /home/phil/edube
flutter create --org com.pp1 --project-name pp1study --platforms android app
```
...
1. `courseware/` → `app/assets/courseware/` (rsync, delete-stale)
2. `tools/export_content.py` → `app/assets/courseware/pack.json`
3. `android-overlay/` → `app/android/` (rsync)
...
`app/android-overlay/app/build.gradle`.
...
| Missing gradle wrapper | run `flutter create … app` again (step 2), do not delete `android/gradle/` |
| `flutter build` can't find assets | run `tools/build_apk.sh` (does the rsync + export) or `rsync -a --delete courseware/ app/assets/courseware/ && python3 tools/export_content.py` |
| Method channel `No implementation found` | the `.aar` wasn't linked — check `app/android/app/libs/goengine-release.aar` exists and `dependencies { implementation files(...) }` is present |
...
- **Dart code**: `cd app && flutter run` (content pack export still needed first: see above).
- **Go engine**: edit `app/go/engine.go` → `REBUILD_GO=1 tools/build_apk.sh`.
- **Python sandbox**: edit `app/android-overlay/app/src/main/python/sandbox.py` → rebuild.